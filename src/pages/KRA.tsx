...
import DynamicKraEvaluation from '@/components/Question&answers';
import ScoreCard from '@/components/ScoreCard';
import { title } from 'process';
import React, { useState } from 'react';
// Note: The original imports for Vision/Values/Tobe are replaced with placeholder components.
// You will need to create thes
// e components or replace them with your actual KRA logic.
const MDheader = "MD's Evaluation";
const Teamheader = "Team's Evaluation";
const data = {

  screenshot: {
    score: { achieved: 21, total: 35 }, // Asegúrate que la estructura sea esta
    title: "Functional Worker",
    message: "Doing tasks but needs stronger accountability."
  },
};
...
function KraSelfEvaluation() {
  ...
  // 2. A simple array of our tab names (updated for KRA)
  const tabs = ['MD', 'Team'];
      ...
      <div className="max-w-4xl p-6 md:p-8 rounded-xl shadow-sm">
        ...
        <div className="flex justify-between items-start mb-8">
          <div className="flex items-center gap-4">
            ...
            {/* Title & Subtitle - Updated for KRA */}
            <div>
        ...
        <div className="w-full bg-purple-50/50 rounded-xl min-h-[300px] border border-purple-100/50">
          ...
          {activeTab === 'MD' && (
            <>
         <ScoreCard data={data.screenshot}  />
         <QuestionsAndAnswers header={MDheader}/>
         </>
          )}
          ...
          {activeTab === 'Team' && (
            <>
         <ScoreCard data={data.screenshot} />
         <QuestionsAndAnswers header={Teamheader}/>
         </>
          )}
        </div>
      </div>
