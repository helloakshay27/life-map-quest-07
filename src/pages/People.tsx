...
interface UpcomingDate { id: string; title: string; date: string; }
interface ReachOut { id: string; name: string; reason: string; }
interface HealthStat { id: string; name: string; score: number; }
...
const People = () => {
    ...
    const fetchPeopleData = async () => {
      try {
        ...
        const mockData = await new Promise<PeopleData>((resolve) =>
          setTimeout(() => resolve({ upcomingDates: [], reachOuts: [], healthStats: [] }), 800)
        );
        setData(mockData);
      } catch (err) {
  ...
  const filteredPeople = people
    .filter((p) => {
      if (relationshipFilter !== "All Relationships" && p.relationship_type !== relationshipFilter) return false;
      if (priorityFilter === "High" && p.importance_level < 4) return false;
      if (priorityFilter === "Medium" && (p.importance_level < 2 || p.importance_level > 3)) return false;
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Nurture your meaningful relationships</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div className="flex min-h-[200px] flex-col rounded-2xl bg-white p-6 dark:bg-purple-950/20 shadow-sm border border-gray-100">
            <div className="mb-6 flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold text-foreground">Upcoming Dates</h3>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4 p-4 border-b border-gray-100 bg-white">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button
                  onClick={() => { setIsRelationshipOpen(!isRelationshipOpen); setIsPriorityOpen(false); }}
                ...
                {isRelationshipOpen && (
                  <div className="absolute top-full left-0 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg z-20 py-1">
                    {["All Relationships", "Family", "Close Friend", "Friend", "Colleague", "Partner", "Mentor", "Acquaintance"].map((opt) => (
                      <div
                        key={opt}
                        onClick={() => { setRelationshipFilter(opt); setIsRelationshipOpen(false); }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button
                  onClick={() => { setIsPriorityOpen(!isPriorityOpen); setIsRelationshipOpen(false); }}
                  className="flex items-center justify-between w-48 px-3 py-2 bg-white border border-gray-200 rounded-md text-sm text-gray-800 hover:bg-gray-50 focus:outline-none"
                >
                  {priorityFilter}
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {!isPeopleLoading && !peopleError && filteredPeople.length === 0 && (
            <div className="flex flex-1 flex-col items-center justify-center p-8 mt-8">
              ...
              <h3 className="text-xl font-medium text-gray-600 mb-6">
                {people.length === 0 ? "No people added yet" : "No results match your filters"}
              </h3>
              {people.length === 0 && (
                <button
                  onClick={() => setIsAddPersonModalOpen(true)}
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-md mt-0.5 ${relationshipColors[person.relationship_type] ?? "bg-gray-100 text-gray-600"}`}>
                  ...
                  {/* Contact */}
                  {(person.contact_info?.phone || person.contact_info?.email) && (
                    <div className="flex flex-col gap-1">
                      {person.contact_info?.phone && (
                        <div className="flex items-center gap-1.5 text-xs text-gray-500">
                          <Phone className="w-3 h-3 shrink-0" />
                          <span className="truncate">{person.contact_info.phone}</span>
                        </div>
                      )}
                      {person.contact_info?.email && (
                        <div className="flex items-center gap-1.5 text-xs text-gray-500">
                          <Mail className="w-3 h-3 shrink-0" />
                          <span className="truncate">{person.contact_info.email}</span>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {person.last_meaningful_interaction && (
                    <p className="text-xs text-gray-400 border-t border-gray-50 pt-2 mt-auto">
                      Last contact:{" "}
                      {new Date(person.last_meaningful_interaction).toLocaleDateString("en-IN", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
