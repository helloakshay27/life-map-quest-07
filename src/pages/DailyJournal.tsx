...
import {
  ...
  Plus, X, Info, Calendar as CalendarIcon, AlertCircle, Loader2
} from "lucide-react";
...
const getToken = (t?: string) => t || localStorage.getItem("auth_token") || "";
  ...
  return (
    <div className="bg-orange-50 rounded-2xl border border-orange-300 p-5 w-full font-sans">
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <span className="font-bold text-[16px] text-gray-900">{formatHeaderDate(selectedDateExternal)}</span>
      </div>
...
const GuidingPrinciples = ({
  coreValues, selectedValues, setSelectedValues, selectedAreas, setSelectedAreas,
    ...
    <div className="bg-purple-50 rounded-2xl border border-purple-200 p-5 w-full font-sans">
      <div className="flex items-center gap-2.5 mb-5">
        <div className="w-10 h-10 rounded-xl bg-purple-500 flex items-center justify-center flex-shrink-0 shadow-sm">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M12 21C12 21 3 14.5 3 8.5a5 5 0 0 1 9-3 5 5 0 0 1 9 3c0 6-9 12.5-9 12.5z" fill="white" />
          </svg>
        </div>
        <span className="font-bold text-[16px] text-gray-900">Guiding Principles</span>
      </div>

      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-semibold text-gray-800">Core Values Lived Today</span>
          <button
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => scroll(valuesScrollRef, -1)} className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-gray-400 hover:text-purple-500 transition-colors">‹</button>
          <div
            ref={valuesScrollRef}
          ...
          >
            {coreValues.length === 0 ? (
              <span className="text-xs text-gray-500 italic px-2">No core values found.</span>
            ) : (
              coreValues.map((valObj) => {
                ...
                return (
                  <button
                    key={valObj.id}
                    onClick={() => toggle(selectedValues, setSelectedValues, valObj.name)}
      ...
      <div>
        <span className="text-sm font-semibold text-gray-800 block mb-2">Life Areas Focused On</span>
        <div className="flex items-center gap-1">
          <button onClick={() => scroll(areasScrollRef, -1)} className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-gray-400 hover:text-purple-500 transition-colors">‹</button>
          <div
            ref={areasScrollRef}
...
const TodaysReflection = ({
  ...
  const addAccomplishment = () =>
    setAccomplishments((prev: any) => [...prev, { id: Date.now(), title: "", checked: false }]);
  ...
  const updateAccomplishment = (id: number, field: string, value: any) =>
    setAccomplishments((prev: any) => prev.map((a: any) => (a.id === id ? { ...a, [field]: value } : a)));
  const toggleMood = (mood: string) =>
    setSelectedMoods((prev: any) => prev.includes(mood) ? prev.filter((m: any) => m !== mood) : [...prev, mood]);
  ...
  return (
    <div className="bg-teal-50/40 rounded-2xl border border-teal-300 p-5 w-full font-sans">
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <span className="font-bold text-[16px] text-gray-900">Today's Reflection</span>
      </div>

      {/* Accomplishments */}
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="font-semibold text-sm text-gray-800">Today's Accomplishments</span>
          <button
        ...
        {accomplishments.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {accomplishments.map((item: any) => (
              <div key={item.id} className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-2.5 shadow-sm">
                <input
                  type="checkbox"
                  checked={item.checked}
                  onChange={(e) => updateAccomplishment(item.id, "checked", e.target.checked)}
                ...
                />
                <button onClick={() => removeAccomplishment(item.id)} className="text-gray-400 hover:text-red-500 transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ))}
      ...
      <div className="bg-white rounded-xl border border-teal-200 overflow-hidden mb-4">
        <div className="flex items-center justify-between px-4 py-3">
          <span className="font-semibold text-sm text-gray-900">Today's Habits</span>
          <button onClick={() => navigate("/goals-habits")} className="text-teal-500 hover:text-teal-700 text-xs font-semibold transition-colors">
            Manage Habits
          </button>
        </div>
        {habits.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-6 px-4 border-t border-gray-100">
            <p className="text-sm text-orange-500 font-medium">No habits scheduled for today</p>
            <button
          ...
          <div className="flex flex-col gap-2 px-3 pb-3">
            {habits.map((habit, idx) => {
              ...
              return (
                <div key={habit.habit_id || idx} className="rounded-xl overflow-hidden border border-gray-200 mt-1">
                  <div
                  ...
                  >
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${habit.completed ? "border-teal-400 bg-teal-400" : "border-teal-300 bg-white"}`}>
                      {habit.completed && (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-800 leading-tight">{habit.name}</p>
                      <div className="flex items-center gap-1 mt-0.5">
                        <span className="text-[12px]">🗓️</span>
                        <span className="text-xs text-gray-500 font-medium">{habit.frequency || "Daily"}</span>
                      </div>
                    </div>
                  ...
                  <div className="px-4 pb-3 pt-1 bg-white border-t border-gray-100">
                    <div className="grid grid-cols-7 gap-1.5">
                      {DAY_LABELS.map((label, i) => (
                        <div key={i} className="text-center text-[10px] font-semibold text-gray-400 mb-1">{label}</div>
                      ))}
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div>
          <p className="text-sm font-medium text-gray-800 mb-1.5">What are you grateful for today?</p>
          <textarea
            rows={3}
            value={gratitude}
        ...
        <div>
          <p className="text-sm font-medium text-gray-800 mb-1.5">Challenges, Changes & Key Insights?</p>
          <textarea
            rows={3}
            value={challenges}
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            style={{ background: `linear-gradient(to right, #111 0%, #111 ${energy * 10}%, #e5e7eb ${energy * 10}%, #e5e7eb 100%)` }}
          />
        </div>
        <div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-semibold text-gray-800">Alignment</span>
            <span className="text-teal-500 font-bold text-sm">{alignment}/10</span>
          </div>
          <input
            type="range" min={0} max={10} value={alignment}
            onChange={(e) => setAlignment(Number(e.target.value))}
            className="w-full h-1.5 rounded-full appearance-none cursor-pointer"
            style={{ background: `linear-gradient(to right, #14b8a6 0%, #14b8a6 ${alignment * 10}%, #e5e7eb ${alignment * 10}%, #e5e7eb 100%)` }}
          />
        </div>
      </div>
...
// ── ShapingTomorrow ───────────────────────────────────────────────────────────
const ShapingTomorrow = ({ priorities, setPriorities }: { priorities: string[]; setPriorities: any }) => {
  ...
  useEffect(() => {
    const fetchCalendars = async () => {
      try {
        ...
        setCalendars(
          Array.isArray(data)
            ? data.map((item) => ({ id: String(item.id), name: item.name, embedUrl: item.embed_url }))
            : []
        );
      } catch {
        setCalendars([]);
  ...
  return (
    <div className="w-full bg-[#F4F9FF] border border-[#E2E8F0] rounded-xl p-6 font-sans shadow-sm">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-[18px] font-bold text-[#1E293B]">Shaping Tomorrow</h2>
      </div>

      <div className="mb-4">
        {isLoading ? (
          <div className="bg-white border border-[#E2E8F0] rounded-lg flex items-center justify-center min-h-[140px] shadow-sm">
            <p className="text-sm font-medium text-gray-400 animate-pulse">Loading calendars...</p>
          </div>
        ) : calendars.length === 0 ? (
          <div className="bg-white border border-[#E2E8F0] rounded-lg flex flex-col items-center justify-center min-h-[140px] p-8 text-center shadow-sm">
            <CalendarIcon className="w-8 h-8 text-[#CBD5E1] mb-3" strokeWidth={1.5} />
            <p className="text-[15px] font-semibold text-[#64748B] mb-1">No calendars configured</p>
            <p className="text-[13px] text-[#94A3B8]">Add calendars in the Calendar page</p>
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            {calendars.map((calendar) => (
              <div key={calendar.id} className="bg-white border border-[#E2E8F0] rounded-lg overflow-hidden shadow-sm">
                <div className="flex items-center gap-2 px-4 py-3 bg-[#F0F6FF] border-b border-[#E2E8F0]">
                  ...
                  <span className="text-sm font-semibold text-[#334155]">{calendar.name}</span>
                </div>
      ...
      <div className="bg-[#FFFCE8] border border-[#FDE047] rounded-md px-4 py-3 mb-6 flex items-center gap-2 shadow-sm">
        ...
        <p className="text-[13.5px] text-[#475569]">
          <span className="font-bold text-[#1E293B]">Review Your 'Why':</span> Does your schedule reflect your long-term goals?
        </p>
      </div>

      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[15px] font-bold text-[#334155]">Priorities for Tomorrow?</h3>
          <button
            onClick={addPriority}
...
// ── DailyAffirmation ──────────────────────────────────────────────────────────
const DailyAffirmation = ({ affirmation, setAffirmation }: { affirmation: string; setAffirmation: any }) => {
  ...
  return (
    <div className="bg-purple-50 rounded-2xl p-6 w-full font-sans">
      ...
      <div className="bg-yellow-50 rounded-xl px-4 py-3 flex items-center justify-between mb-3 min-h-[66px]">
        <button onClick={prev} disabled={loading || total === 0} className="text-purple-500 text-2xl font-light px-2 hover:text-purple-700 disabled:opacity-30 transition-colors">‹</button>
        <div className="flex-1 text-center">
          {loading ? (
            ...
            <>
              <div className="text-purple-500 font-semibold text-xs mb-1">Affirmation ({current + 1} of {total})</div>
              <div className="text-purple-700 italic text-sm font-medium">{getText(affirmationsList[current])}</div>
            </>
          )}
        </div>
        <button onClick={next} disabled={loading || total === 0} className="text-purple-500 text-2xl font-light px-2 hover:text-purple-700 disabled:opacity-30 transition-colors">›</button>
      </div>
      ...
      <div className="bg-violet-100 rounded-lg px-4 py-2.5 flex items-center gap-2">
        ...
        <span className="text-violet-700 text-xs italic">Present tense ("I am"), positive, specific, repeat daily with emotion.</span>
      </div>
    </div>
  );
};
...
// ── ReviewGoals ───────────────────────────────────────────────────────────────
const ReviewGoals = () => (
  <div className="relative w-full bg-[#F3F5FF] border border-[#6366F1] rounded-[12px] p-5 font-sans overflow-hidden shadow-sm">
    <div className="absolute -top-12 -right-12 w-48 h-48 bg-white/60 blur-3xl rounded-full pointer-events-none"></div>
    <div className="flex items-center gap-3 mb-5 relative z-10">
      <div className="w-[42px] h-[42px] bg-[#6366F1] rounded-[10px] shadow-sm flex items-center justify-center">
        <div className="relative flex items-center justify-center w-[22px] h-[22px]">
          <div className="absolute w-full h-full rounded-full border-[1.5px] border-white"></div>
          <div className="absolute w-[55%] h-[55%] rounded-full border-[1.5px] border-white"></div>
          <div className="absolute w-[20%] h-[20%] rounded-full bg-white"></div>
        </div>
      </div>
      <h2 className="text-[17px] font-bold text-[#1E293B] tracking-tight">Review Goals</h2>
    </div>
    <div className="bg-white rounded-[10px] py-[48px] px-6 flex flex-col items-center justify-center relative z-10">
      <div className="relative flex items-center justify-center w-10 h-10 mb-4">
        <div className="absolute w-full h-full rounded-full border-[2px] border-[#A5B4FC]"></div>
        <div className="absolute w-[55%] h-[55%] rounded-full border-[2px] border-[#A5B4FC]"></div>
        <div className="absolute w-[20%] h-[20%] rounded-full bg-[#A5B4FC]"></div>
      </div>
      <p className="text-[15px] font-medium text-[#64748B] mb-1.5">No goals to review.</p>
      <p className="text-[13px] text-[#94A3B8]">Set goals in your Weekly Journal to see them here.</p>
    </div>
  </div>
);
...
const AddDreamModal = ({ onClose, onAdd }: any) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 transition-colors"><X className="w-4 h-4" /></button>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
          <button
...
const BucketListProgress = () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          const mapCat = (arr: any, pLabel: string) => {
            if (Array.isArray(arr)) arr.forEach((item) => mapped.push({ id: item.id.toString(), title: item.title || "", category: item.category || "Personal", progress: pLabel }));
          };
  ...
  const handleCategoryChange = async (id: string, val: string) => {
    ...
    fetch(`${API_BASE}/dreams/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("auth_token")}` },
      body: JSON.stringify({ title: bucketList.find((i) => i.id === id)?.title, category: val }),
    }).catch(() => sonnerToast.error("Failed update"));
  };
  ...
  return (
    <>
      {showAddModal && (
        <AddDreamModal onClose={() => setShowAddModal(false)} onAdd={(i: any) => setBucketList((p) => [i, ...p])} />
      )}
      <div className="bg-amber-50 rounded-2xl p-5 border border-amber-200 font-sans w-full">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <select value={progressFilter} onChange={(e) => setProgressFilter(e.target.value)} className="appearance-none h-8 pl-3 pr-7 rounded-lg border border-gray-200 bg-white text-xs text-gray-700 outline-none">
            ...
            <div className="relative">
              <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="appearance-none h-8 pl-3 pr-7 rounded-lg border border-gray-200 bg-white text-xs text-gray-700 outline-none">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-3 max-h-[380px] overflow-y-auto pr-0.5">
            {filtered.length > 0 ? (
              filtered.map((item) => (
                <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm px-4 pt-3 pb-3 flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setUpdateTexts((p: any) => ({ ...p, [item.id]: e.target.value }))}
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <PillSelect value={item.progress} options={PROGRESS_OPTIONS} onChange={(val: string) => handleProgressChange(item.id, val)} colorFn={getProgressStyle} />
                    <PillSelect value={item.category} options={CATEGORY_OPTIONS} onChange={(val: string) => handleCategoryChange(item.id, val)} colorFn={getCategoryStyle} />
                  </div>
                </div>
              ))
            ) : (
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <span className="font-semibold text-gray-900 text-sm flex-1 min-w-0 truncate">{dateLabel}</span>
        <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-orange-50 text-orange-500 border border-orange-100 whitespace-nowrap">Energy {journal.energy_score}/10</span>
        <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-teal-50 text-teal-600 border border-teal-100 whitespace-nowrap">Align {journal.alignment_score}/10</span>
        <button
          onClick={(e) => { e.stopPropagation(); onEdit(journal.id); }}
        ...
        <button onClick={handleToggle} className="p-1.5 text-gray-400 hover:text-gray-700">
      ...
      {expanded && (
        <div className="px-5 pb-5 flex flex-col gap-3 border-t border-gray-50 pt-3">
          ...
          {detail && (
            <>
              {detail.core_values_snapshot && detail.core_values_snapshot.length > 0 && (
                <div className="rounded-xl bg-purple-50 border border-purple-100 px-4 py-3">
                  <p className="text-xs font-bold text-purple-700 mb-2">Core Values Lived Today</p>
                  <div className="flex flex-wrap gap-1.5">
                    {detail.core_values_snapshot.map((v, i) => (
                      <span key={i} className="px-2.5 py-1 rounded-full bg-purple-500 text-white text-[11px] font-semibold shadow-sm">{v}</span>
                    ))}
                  </div>
                </div>
              )}
              {detail.data?.selected_life_areas && detail.data.selected_life_areas.length > 0 && (
                <div className="rounded-xl bg-indigo-50 border border-indigo-100 px-4 py-3">
                  <p className="text-xs font-bold text-indigo-700 mb-2">Life Areas Focused On</p>
                  <div className="flex flex-wrap gap-1.5">
                    {detail.data.selected_life_areas.map((a, i) => (
                      <span key={i} className="px-2.5 py-1 rounded-full bg-indigo-100 text-indigo-700 text-[11px] font-semibold border border-indigo-200">{a}</span>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div className="rounded-xl bg-purple-50 border border-purple-100 px-4 py-3">
                  <div className="flex items-center gap-2 mb-1.5">
                    <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center flex-shrink-0">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="white"><path d="M12 2L13.5 8.5L20 10L13.5 11.5L12 18L10.5 11.5L4 10L10.5 8.5L12 2Z" /></svg>
                    </div>
                    <span className="text-sm font-semibold text-purple-800">Daily Affirmation</span>
                  </div>
                  <p className="text-sm text-purple-700 italic">"{detail.affirmation}"</p>
                </div>
              )}
              {detail.priorities && detail.priorities.length > 0 && (
                <div className="rounded-xl bg-blue-50 border border-blue-100 px-4 py-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-semibold text-blue-800">Priorities for Tomorrow</span>
                  </div>
                  <ul className="space-y-1">
                    {detail.priorities.map((p, i) => (
                      <li key={i} className="text-sm text-blue-700 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-blue-400 flex-shrink-0" />{p}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {detail.gratitude_note && (
                    <div className="rounded-xl bg-yellow-50 border border-yellow-100 px-4 py-3">
                      <p className="text-xs font-bold text-yellow-700 mb-1">Gratitude</p>
                      <p className="text-sm text-yellow-800">{detail.gratitude_note}</p>
                    </div>
                  )}
                  {detail.challenges_note && (
                    <div className="rounded-xl bg-indigo-50 border border-indigo-100 px-4 py-3">
                      <p className="text-xs font-bold text-indigo-700 mb-1">Challenges & Insights</p>
                      <p className="text-sm text-indigo-800">{detail.challenges_note}</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="rounded-xl bg-pink-50 border border-pink-100 px-4 py-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {detail.mood_tags.map((m, i) => (
                      <span key={i} className="px-2.5 py-1 rounded-full bg-pink-100 text-pink-700 text-[11px] font-semibold">{m}</span>
                    ))}
                  </div>
                </div>
              )}
              {detail.accomplishments && detail.accomplishments.length > 0 && (
                <div className="rounded-xl bg-green-50 border border-green-100 px-4 py-3">
                  <p className="text-xs font-bold text-green-700 mb-2">Accomplishments</p>
                  <ul className="space-y-1">
                    {detail.accomplishments.map((a, i) => (
                      <li key={i} className="text-sm text-green-800 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-green-400 flex-shrink-0" />{a.title}
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {detail.habits_snapshot.map((h, i) => (
                      <div key={i} className="flex items-center justify-between">
                        ...
                        <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${h.completed ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
              ...
              {detail.bucket_updates && detail.bucket_updates.length > 0 && (
                <div className="rounded-xl bg-amber-50 border border-amber-100 px-4 py-3">
                  <p className="text-xs font-bold text-amber-700 mb-2">Bucket List Updates</p>
                  {detail.bucket_updates.map((u, i) => (
                    <div key={i} className="mb-1.5">
                      <p className="text-sm font-semibold text-amber-800">{u.title}</p>
              ...
              {detail.todos_snapshot && detail.todos_snapshot.length > 0 && (
                <div className="rounded-xl bg-slate-50 border border-slate-100 px-4 py-3">
                  <p className="text-xs font-bold text-slate-700 mb-2">To-Dos Snapshot</p>
                  <div className="flex flex-col gap-1.5">
                    {detail.todos_snapshot.map((t, i) => (
                      <div key={i} className="flex items-center justify-between">
                        <span className="text-sm text-slate-800">{t.title}</span>
                        <div className="flex gap-1.5">
                          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{t.priority}</span>
                          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-slate-200 text-slate-700">{t.status}</span>
                        </div>
                      </div>
                    ))}
                  </div>
...
const PastLetterRow = ({
  ...
  const dateLabel =
    ...
    (letter.written_on ? format(new Date(letter.written_on), "EEEE, MMMM d, yyyy") : "");
  ...
  return (
    <div className="border-b border-gray-100 last:border-b-0">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-gray-800 truncate">{letter.subject || "Dear Future Me"}</p>
  ...
  useEffect(() => {
    ...
    const fetchDateData = async () => {
      ...
      try {
        ...
        const jRes = await fetch(`${LIFE_API}/user_journals/0?date=${d}&journal_type=daily`, {
          headers: { Authorization: `Bearer ${getToken(token)}` },
        });
        ...
        if (existingJournal) {
          ...
          setAchievements(
            existingJournal.accomplishments?.map((a: any, i: number) => ({ id: i, title: a.title, checked: true })) || []
          );
          ...
          const enrichedHabits = savedHabits.map((sh: any) => {
            ...
            return {
              ...
              week_history: match?.week_history || match?.weekly_completion || [],
            };
          });
  ...
  useEffect(() => {
    ...
    if (activeTab === "letters") {
      ...
      fetch(`${LIFE_API}/user_letters`, { headers: { Authorization: `Bearer ${getToken(token)}` } })
        .then((r) => r.json())
        .then((d) => setPastLetters(Array.isArray(d) ? d : d?.user_letters ?? []))
        .catch(() => {})
      ...
      <div className="w-full mx-auto">
        ...
        {isEditMode && (
          <div className="mb-4 px-4 py-3 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-between">
            ...
            <button
              onClick={() => { setIsEditMode(false); setSelectedDate(new Date(selectedDate)); }}
            ...
            {isLoadingDaily ? (
              <div className="flex items-center justify-center py-32">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm text-gray-500 font-medium">Loading {format(selectedDate, "MMMM d, yyyy")}...</p>
                </div>
              </div>
            ) : (
              <div className="flex flex-col space-y-8 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                />
                <TodaysReflection
                  accomplishments={achievements} setAccomplishments={setAchievements}
                  gratitude={gratitude} setGratitude={setGratitude}
                  challenges={challenges} setChallenges={setChallenges}
                  selectedMoods={selectedMoods} setSelectedMoods={setSelectedMoods}
                  energy={energy} setEnergy={setEnergy}
                  alignment={alignment} setAlignment={setAlignment}
                  habits={habitsSnapshot} setHabits={setHabitsSnapshot}
                  selectedDate={selectedDate}
                />
                <ShapingTomorrow priorities={priorities} setPriorities={setPriorities} />
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-12 text-center shadow-sm">
              {isLoadingInsights ? (
                ...
                <>
                  ...
                  <h3 className="text-lg font-bold text-gray-900 mb-2">{insightsData.message}</h3>
                  {insightsData.hint && <p className="text-sm text-gray-500 max-w-sm mx-auto">{insightsData.hint}</p>}
                </>
              ) : (
                <>
                  <div className="mb-4 text-5xl">💡</div>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-900">
                        {editingLetter ? "Edit Your Letter" : "Write a Letter to Yourself"}
                      </h3>
                      <p className="text-sm text-gray-500">Share your thoughts, dreams, and reflections</p>
                    </div>
                  </div>
                  {editingLetter && (
                    <button
                      onClick={() => { setEditingLetter(null); setLetterSubject(""); setLetterBody(""); }}
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-800 mb-1.5 block">Subject (Optional)</label>
                    <Input
                  ...
                  <div>
                    <label className="text-sm font-semibold text-gray-800 mb-1.5 block">Your Letter</label>
                    <Textarea
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
                <div className="mb-4">
                  <h3 className="text-lg font-bold text-gray-900">Past Letters</h3>
                  <p className="text-sm text-gray-400 mt-0.5">Click to expand and read your past letters</p>
                </div>
                {isLoadingLetters ? (
                  <p className="text-gray-500 text-center py-4">Loading past letters...</p>
                ) : pastLetters.length === 0 ? (
                  <p className="text-gray-500 text-center py-4">No past letters found. Write your first one above!</p>
                ) : (
                  <div className="divide-y divide-gray-100">
                    {pastLetters.map((l) => (
                      <PastLetterRow
                        ...
                        letter={l}
                        token={token}
                        onDelete={(id) => setPastLetters((p) => p.filter((x) => x.id !== id))}
