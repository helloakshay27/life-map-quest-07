...
const PastWeeklyJournalRow = ({
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    try {
      const res = await apiRequest(`/user_journals/${journal.id}`, { method: "DELETE" });
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-gray-900 truncate text-sm">{dateLabel}</p>
          {journal.start_date && (
            <p className="text-xs text-gray-400 mt-0.5">
              Week of{" "}
              {format(new Date(journal.start_date), "MMM d")} –{" "}
              {format(endOfWeek(new Date(journal.start_date), { weekStartsOn: 0 }), "MMM d, yyyy")}
            </p>
          )}
        </div>
        ...
        <div className="px-5 pb-5 flex flex-col gap-3 border-t border-gray-50 pt-3">
          {journal.gratitude_note && (
            <div className="rounded-xl bg-yellow-50 border border-yellow-100 px-4 py-3">
              ...
              <p className="text-sm text-yellow-800 italic">"{journal.gratitude_note}"</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {journal.data?.biggest_challenge && (
              <div className="rounded-xl bg-indigo-50 border border-indigo-100 px-4 py-3">
                <p className="text-xs font-bold text-indigo-700 mb-1">Biggest Challenge</p>
                <p className="text-sm text-indigo-800">{journal.data.biggest_challenge}</p>
            ...
            {journal.data?.key_insight && (
              <div className="rounded-xl bg-pink-50 border border-pink-100 px-4 py-3">
                <p className="text-xs font-bold text-pink-700 mb-1">Key Insight</p>
                <p className="text-sm text-pink-800">{journal.data.key_insight}</p>
              </div>
            )}
          </div>
          ...
          {journal.data?.weekly_story && (
            <div className="rounded-xl bg-gray-50 border border-gray-100 px-4 py-3">
              <p className="text-xs font-bold text-gray-600 mb-1">Weekly Story</p>
              <p className="text-sm text-gray-700">{journal.data.weekly_story}</p>
            </div>
          )}

          {journal.data?.mission_connection && (
            <div className="rounded-xl bg-emerald-50 border border-emerald-100 px-4 py-3">
              <p className="text-xs font-bold text-emerald-700 mb-1">Mission Connection</p>
              <p className="text-sm text-emerald-800">{journal.data.mission_connection}</p>
            </div>
          )}

          {journal.data?.wins && journal.data.wins.length > 0 && (
            <div className="rounded-xl bg-green-50 border border-green-100 px-4 py-3">
              <p className="text-xs font-bold text-green-700 mb-2">Weekly Wins</p>
                ...
                {journal.data.wins.map((win, i) => (
                  ...
                  >
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-gray-800">{win.description}</p>
  ...
  useEffect(() => {
    if (activeTab === "insights") {
      const fetchInsights = async () => {
        ...
        try {
          const res = await apiRequest("/user_journals/weekly_journals_insights");
  ...
  const handleSavePlan = async () => {
    ...
    if (weekStart > today) {
      return toast({
        ...
        description: "You can only create weekly journals for the current or past weeks.",
        variant: "destructive",
      });
    }
    ...
    if (journalId && !isEditMode) {
      return toast({
        ...
        description: "Use the Update button in the Past tab to edit this entry.",
        variant: "destructive",
      });
    }
    ...
    try {
      ...
      toast({
        title: isUpdate ? "Weekly Journal Updated ✅" : "Weekly Journal Saved ✅",
        ...
        <div className="px-4 py-3 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-blue-700">
              ...
              {format(endOfWeek(currentDate, { weekStartsOn: 0 }), "MMM d, yyyy")}
            </span>
          </div>
          <button
            onClick={() => {
              ...
              {isLoadingDaily ? (
                <div className="py-20 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <p className="text-gray-500 font-medium">Loading journal for this week...</p>
                  </div>
                </div>
              ) : (
                <>
              ...
              <div className="grid grid-cols-1 gap-3">
                {pastJournals
                  .filter((journal) => {
                    ...
                    return isSameWeek(new Date(journal.start_date), calendarDate, {
                      weekStartsOn: 0,
                    });
                  })
                  .map((journal) => (
          ...
          <TabsContent value="insights" className="focus:outline-none">
            {isLoadingInsights ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    <p className="text-sm font-semibold text-gray-500 mb-1">Total Weeks</p>
                    <p className="text-3xl font-bold text-gray-900">{insightsData.total_weeks}</p>
                  </div>
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    <p className="text-sm font-semibold text-gray-500 mb-1">Avg Alignment</p>
                    <p className="text-3xl font-bold text-blue-600">
                      {insightsData.alignment_average?.toFixed(1) || "-"}
                    </p>
                  </div>
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    <p className="text-sm font-semibold text-gray-500 mb-1">Avg Balance</p>
                    <p className="text-3xl font-bold text-purple-600">
                      {insightsData.average_life_balance?.toFixed(1) || "-"}
                    </p>
                  </div>
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    <p className="text-sm font-semibold text-gray-500 mb-1">Top Day</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {insightsData.common_challenges &&
                    insightsData.common_challenges.length > 0 && (
                      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 className="text-lg font-bold text-gray-900 mb-4">Common Challenges</h3>
                  ...
                  {insightsData.category_distribution &&
                    Object.keys(insightsData.category_distribution).length > 0 && (
                      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 className="text-lg font-bold text-gray-900 mb-4">Win Categories</h3>
                        <ul className="space-y-3">
              ...
              <div className="py-20 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
                ...
                <p className="text-lg font-bold text-gray-900">{insightsData.message}</p>
                {insightsData.hint && (
                  <p className="text-sm text-gray-500 mt-2">{insightsData.hint}</p>
                )}
              </div>
            ) : (
              <div className="py-20 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
