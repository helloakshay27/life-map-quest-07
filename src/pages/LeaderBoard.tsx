        ...
        if (activeTab === "Rankings") {
          ...
          const parseCount = async (settledResult) => {
            ...
            try {
              ...
              // Handle standard array and object responses
              ...
              // Handle the specific grouped "dreams" response
              if (
                json.dreaming ||
                json.planning ||
                json.in_progress ||
                ...
                const dreamingCount = Array.isArray(json.dreaming)
                  ? json.dreaming.length
                  : 0;
                const planningCount = Array.isArray(json.planning)
                  ? json.planning.length
                  : 0;
                const inProgressCount = Array.isArray(json.in_progress)
                  ? json.in_progress.length
                  : 0;
                const achievedCount = Array.isArray(json.achieved)
                  ? json.achieved.length
                  : 0;
          ...
          const [dailyCount, weeklyCount, goalsCount, dreamsCount] =
            await Promise.all([
              parseCount(dailyRes),
                  ...
                  },
                  {
                    ...
                    iconColor:
                      setupCompleted === setupTotal
                        ? "text-green-500"
                        : "text-slate-400",
                    value: `${setupCompleted}/${setupTotal}`,
                    label: "Setup",
                  },
                ],
              }
