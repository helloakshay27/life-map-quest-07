...
const BucketListProgress = () => {
  ...
  const handleProgressChange = async (id, newProgress) => {
    ...
    try {
      const token = localStorage.getItem("auth_token");
      const res = await fetch(`${API_BASE}/dreams/${id}/change_status`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if(res.ok){
         toast.success(`Status updated to ${newProgress}`, {
           style: { background: "#0B5041", color: "#fff", border: "none" }
         });
      } else {
  ...
  const handleCategoryChange = async (id, newCategory) => {
    ...
    try {
      const token = localStorage.getItem("auth_token");
      const res = await fetch(`${API_BASE}/dreams/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleAddUpdate = async (id) => {
    ...
    try {
      const token = localStorage.getItem("auth_token");
      const res = await fetch(`${API_BASE}/dreams/${id}/add_note`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (res.ok) {
        ...
        toast.error("Failed to add note on server", {
          style: { background: "#A32D2D", color: "#fff", border: "none" }
        });
      }
    } catch (error) {
